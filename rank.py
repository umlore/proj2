...
def rankUrls(query, page_ranks, index):
    ...
    sorted_adocs = sorted(all_adocs, key=operator.attrgetter(
        'total_tokens'), reverse=True)
...
def search():
    ...
    #
    inverted_index_json = requests.post(
        'https://teamy.cs.rpi.edu/index', data=index_json_request, headers=headers, timeout=1.000)
    ...
    if(inverted_index_json['returnCode'] != 0):
        print("An error occured (", inverted_index_json['error'],
              ") with return code", inverted_index_json['returnCode'])
    ...
    page_rank_json = requests.post(
        'https://teamqq.cs.rpi.edu/pageRank', data=page_rank_request, headers=headers, timeout=1.000)
